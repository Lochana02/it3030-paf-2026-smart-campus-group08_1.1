...
const DashboardPage = () => {
  ...
  const availablePercent = resources.total > 0 ? ((resources.available / resources.total) * 100).toFixed(1) : 0
  const inUsePercent = resources.total > 0 ? ((resources.inUse / resources.total) * 100).toFixed(1) : 0
  const maintenancePercent = resources.total > 0 ? ((resources.maintenance / resources.total) * 100).toFixed(1) : 0
  const bookedPercent = resources.total > 0 ? ((resources.booked / resources.total) * 100).toFixed(1) : 0
  ...
  return (
    <div style={{ 
      ...
      {/* Two Column Layout - Distribution & Stats */}
      <div style={{
        display: 'grid',
        ...
        {/* Left - Resource Status Distribution */}
        <div style={{
          backgroundColor: 'white',
          borderRadius: '16px',
          padding: '24px',
          ...
          <div style={{ marginBottom: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontSize: '13px' }}>
              <span>✅ Available</span>
              <span style={{ color: '#6c757d' }}>{resources.available} ({availablePercent}%)</span>
            </div>
            <div style={{ 
          ...
          <div style={{ marginBottom: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontSize: '13px' }}>
              <span>⚡ In Use</span>
              <span style={{ color: '#6c757d' }}>{resources.inUse} ({inUsePercent}%)</span>
            </div>
            <div style={{ backgroundColor: '#e9ecef', borderRadius: '6px', height: '8px', overflow: 'hidden' }}>
              <div style={{ width: `${inUsePercent}%`, backgroundColor: '#ffc107', height: '100%', borderRadius: '6px' }}></div>
            </div>
          </div>
          ...
          <div style={{ marginBottom: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontSize: '13px' }}>
              ...
              <span style={{ color: '#6c757d' }}>{resources.maintenance} ({maintenancePercent}%)</span>
            </div>
            <div style={{ backgroundColor: '#e9ecef', borderRadius: '6px', height: '8px', overflow: 'hidden' }}>
              <div style={{ width: `${maintenancePercent}%`, backgroundColor: '#dc3545', height: '100%', borderRadius: '6px' }}></div>
            </div>
          </div>
          ...
          <div style={{ marginBottom: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontSize: '13px' }}>
              <span>📖 Booked</span>
              <span style={{ color: '#6c757d' }}>{resources.booked} ({bookedPercent}%)</span>
            </div>
            <div style={{ backgroundColor: '#e9ecef', borderRadius: '6px', height: '8px', overflow: 'hidden' }}>
              <div style={{ width: `${bookedPercent}%`, backgroundColor: '#17a2b8', height: '100%', borderRadius: '6px' }}></div>
            </div>
          </div>
        </div>

        {/* Right - Booking Statistics */}
        <div style={{
          backgroundColor: 'white',
          borderRadius: '16px',
          padding: '24px',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '14px', color: '#6c757d' }}>📊 Total Bookings</span>
              <span style={{ fontSize: '18px', fontWeight: 'bold', color: '#1a1a2e' }}>{stats?.totalBookings || 0}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '14px', color: '#6c757d' }}>⏳ Pending Approval</span>
              <span style={{ fontSize: '18px', fontWeight: 'bold', color: '#ffc107' }}>{stats?.totalPending || 0}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '14px', color: '#6c757d' }}>✅ Approved Today</span>
              <span style={{ fontSize: '18px', fontWeight: 'bold', color: '#28a745' }}>{stats?.totalApproved || 0}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '14px', color: '#6c757d' }}>❌ Rejected</span>
              <span style={{ fontSize: '18px', fontWeight: 'bold', color: '#dc3545' }}>{stats?.totalRejected || 0}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '14px', color: '#6c757d' }}>🚫 Cancelled</span>
              <span style={{ fontSize: '18px', fontWeight: 'bold', color: '#6c757d' }}>{stats?.totalCancelled || 0}</span>
            </div>
            <div style={{ 
              marginTop: '10px', 
              paddingTop: '15px', 
              ...
              <span style={{ fontSize: '14px', fontWeight: '600', color: '#1a1a2e' }}>📈 Total Utilization</span>
              <span style={{ fontSize: '18px', fontWeight: 'bold', color: '#0d6efd' }}>{utilizationPercentage}%</span>
            </div>
          </div>
        </div>
      </div>
